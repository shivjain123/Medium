...
df = p.read_csv("D:/(4) WhiteHatJr. Codes/Third Module/Sampling/Project/Medium csv/medium_data.csv")

responses = df["responses"].tolist()

figure = pff.create_distplot([responses], ["Results"], show_hist=False)

#figure.show()

population_mean = s.mean(responses)
population_sd = s.stdev(responses)
...
def random_set_of_mean(counter):
    ...
    for d in range(counter):
        random_i = r.randint(0, len(responses) - 1)
        data_set.append(responses[random_i])
...
def draw_figure(data_set_n):
    ...
    figure.add_trace(go.Scatter(x=[population_mean, population_mean], y=[0, 0.005], mode="lines", name="Mean"))
    figure.show()
...
def setup():
    ...
    print()
    print(
        "Standard Deviation of the Sampling Distribution {f}".format(f=final_sd))
    print()
